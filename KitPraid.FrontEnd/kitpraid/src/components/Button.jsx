...
const Button = ({
  ...
  const renderIcon = (icon, extraProps = {}) => {
    ...
    if (React.isValidElement(icon)) {
      const merged = {
        ...
        color: icon.props.color ?? (hasBackground && !(isHovered || isDisabled) ? '#FFFFFF' : resolvedColor),
    ...
    if (typeof icon === 'function') {
      return React.createElement(icon, {
        ...
        color: hasBackground && !(isHovered || isDisabled) ? '#FFFFFF' : resolvedColor,
        style: { flexShrink: 0 },
        ...extraProps,
      });
    }
  ...
  const computedStyles = useMemo(() => {
    ...
    const backgroundColor = !isDisabled && isHovered ? hoverBackground : baseBackground;
    const textColor = !isDisabled && isHovered ? resolvedColor : (hasBackground ? '#FFFFFF' : resolvedColor);
    ...
    return {
      ...
      backgroundColor,
      color: textColor,
      transition: 'filter 120ms ease, transform 120ms ease, background-color 150ms ease, box-shadow 150ms ease',
      transform: !isDisabled && isHovered ? 'translateY(-1px)' : 'none',
      boxShadow: !isDisabled && isHovered ? '0 4px 10px rgba(0,0,0,0.08)' : 'none',
  ...
  }, [resolvedColor, hasBackground, hasBorder, isDisabled, isBold, sizeStyles, style, isHovered]);
  ...
  const IconCircle = () => (
    <svg
      ...
      style={{ display: 'block' }}
    >
      <circle cx="10" cy="10" r="7.5" stroke={hasBackground ? '#FFFFFF' : resolvedColor} strokeWidth="2" />
    </svg>
  );
  ...
  const IconArrow = () => (
    <svg
      ...
      style={{ display: 'block' }}
    >
      <path d="M5 12h12" stroke={hasBackground ? '#FFFFFF' : resolvedColor} strokeWidth="2" strokeLinecap="round" />
      <path d="M13 6l6 6-6 6" stroke={hasBackground ? '#FFFFFF' : resolvedColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
